...
## Нагрузочное тестирование: Locust (Спринты 22–23)

**Файл:** `load/locustfile.py`
**Инструмент:** Locust 2.46.1
**Цель:** проверить производительность `POST /v1/auth/register` под нагрузкой 20 виртуальных пользователей
**Запуск:** `locust -f load/locustfile.py --host <url>` → `localhost:8089`

| Сценарий | Вес (@task) | Email | Ожидаемый ответ |
|---|---|---|---|
| `register_new_user` | 3 (75% трафика) | `loadtest-{uuid}@test.com` | 201 Created |
| `register_duplicate` | 1 (25% трафика) | `loadtest-duplicate@test.com` | 409 Conflict |
...
| p95 | 32ms |
| p99 | 260ms |
| Failures | 99% — rate limiter (429) |

**Находка:** `POST /v1/auth/register` защищён rate limiter'ом. При ~10 RPS с одного IP сервер возвращает `429 Too Many Requests` с сообщением *«Слишком много попыток регистрации. Повторите через 59 минут»*. Это корректная защита от спам-регистраций, а не баг.

На графике это видно наглядно: в момент включения rate limiter'а время ответа мгновенно падает с ~1500ms до ~0ms — сервер отдаёт 429 без обработки запроса.

**Прогон 2 — Localhost (localhost:8001, Docker dev-стек):**
...
**Вывод из сравнения двух прогонов:**
- Реальная скорость FastAPI-бэкенда: **3ms** (видна в локальном прогоне)
- Сетевая задержка до staging: ~20ms (разница 24ms − 3ms)
- Rate limiter работает корректно и защищает эндпоинт регистрации

**Ключевые концепции Locust:**
- `@task(weight)` — задаёт вероятность выбора сценария (3:1 → 75%/25%)
- `between(1, 3)` — пауза между запросами, имитирует реального пользователя
- `catch_response=True` — ручное управление success/failure независимо от HTTP-статуса
- `name=` — группирует запросы в отчёте по имени, а не URL
- 429 — не баг системы, а security-фича: помечаем как success в catch_response

**Скрины:** `../evidence/sprint-22-23/`
